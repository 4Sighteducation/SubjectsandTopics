...
# Hardcoded PDF URL overrides for subjects with incorrect/missing URLs
HARDCODED_PDF_URLS = {
    "English Language": "https://www.eduqas.co.uk/media/10ea1en0/eduqas-gcse-english-language-from-2015-e.pdf",
    "English Literature": "https://www.eduqas.co.uk/media/x2zpzwgt/eduqas-gcse-english-lit-spec-from-2015-e.pdf",
    "Geography A": "https://www.eduqas.co.uk/media/j0zo4wbh/eduqas-gcse-geography-a-spec-from-2016-e-24-01-20.pdf",
    "Geography B": "https://www.eduqas.co.uk/media/5ofdo23l/gcse-geog-b-spec.pdf",
    "Geology": "https://www.eduqas.co.uk/media/zhun0mwl/eduqas-gcse-geology-spec.pdf",
    "Mathematics": "https://www.eduqas.co.uk/media/cosjcbs1/eduqas-gcse-maths-spec-from-2015-e.pdf",
    "Sociology": "https://www.eduqas.co.uk/media/jk2duibm/eduqas-gcse-sociology-spec-from-2017-eng.pdf"
}
...
class EduqasGCSEUniversalScraper:
                ...
                if separator:
                    ...
                    if len(parts) >= 2:
                        ...
                        if level_part == "GCSE" or level_part == "GCSE / A":
                            # Check hardcoded URLs first, then cached URLs
                            pdf_url = HARDCODED_PDF_URLS.get(subject, pdf_urls.get(subject, ''))
                            subjects.append({
                                'name': subject,
                                'level': 'GCSE',
                                'pdf_url': pdf_url,
    ...
    def scrape_all(self, subject_filter: Optional[str] = None, limit: Optional[int] = None, subjects_list: Optional[List[str]] = None):
        ...
        # If specific subjects list provided, filter to only those
        if subjects_list:
            subject_names_lower = [s.lower() for s in subjects_list]
            self.subjects = [s for s in self.subjects if s['name'].lower() in subject_names_lower]
            if not self.subjects:
                print(f"[ERROR] No subjects found matching: {', '.join(subjects_list)}")
                return False
            print(f"[INFO] Running only specified subjects: {', '.join(subjects_list)}")
        ...
        if not subject_filter and not subjects_list:
        ...
        for i, subject in enumerate(self.subjects, 1):
            ...
            try:
                # STEP 1: Find PDF URL - check hardcoded first, then scrape
                print(f"[INFO] Step 1: Finding PDF URL for {subject['name']}...")
                
                # Check hardcoded URLs first
                if subject['name'] in HARDCODED_PDF_URLS:
                    subject['pdf_url'] = HARDCODED_PDF_URLS[subject['name']]
                    print(f"[OK] Using hardcoded PDF URL for {subject['name']}")
                elif not subject.get('pdf_url'):
                    # Find PDF URL using PDF URL scraper
                    try:
                    ...
                    print(f"[OK] Using cached PDF URL for {subject['name']}")
...
def main():
    ...
    parser.add_argument('--subject', type=str, help='Filter by subject name (partial match)')
    parser.add_argument('--subjects', type=str, help='Comma-separated list of specific subjects to run (e.g., "English Language,Geography A,Geography B")')
    ...
    success = scraper.scrape_all(subject_filter=args.subject, limit=args.limit, subjects_list=subjects_list)
