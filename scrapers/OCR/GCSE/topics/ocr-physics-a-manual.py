"""
OCR GCSE Physics A (Gateway Science) Manual Scraper
===================================================

Extracts topics from OCR GCSE Physics A (Gateway Science) J249 specification.

Structure (same as Biology A):
- Level 0: Subject
- Level 1: Topics (P1-P6, P7 for practical skills)
  - Format: "Topic P1: Matter"
- Level 2: Sub-topics
  - Format: "P1.1 The particle model"
- Level 3: Learning outcomes (with reference codes)
  - Format: "P1.1a", "P1.1b", etc.
...
- P7: Practical activity skills

CRITICAL: Skip "Content Overview" and "Assessment Overview" sections.
Extract only from "2c. Content of topics P1 to P6" section.

Requirements:
    pip install requests pdfplumber openai anthropic

Usage:
    python ocr-physics-a-manual.py
"""
...
PDF_URL = 'https://www.ocr.org.uk/Images/234600-specification-accredited-gcse-gateway-science-suite-physics-a-j249.pdf'
SUBJECT_CODE = 'J249'
SUBJECT_NAME = 'Physics A'
...
class PhysicsAScraper:
    """Scraper for OCR GCSE Physics A (Gateway Science)."""
    ...
    def scrape_all(self):
        """Scrape all topics."""
        print("\n" + "⚛️ "*40)
        print("OCR GCSE PHYSICS A (GATEWAY SCIENCE) SCRAPER")
        print("⚛️ "*40)
        ...
        if not content_section:
            print("[ERROR] Could not find content section (2c. Content of topics P1 to P6)")
            return False
        ...
        # Extract each topic (P1-P7)
        for topic_idx, topic_name in enumerate(TOPICS):
            topic_code = topic_name.split(':')[0].strip()  # e.g., "P1"
            print(f"\n{'='*80}")
    ...
    def _find_content_section(self) -> Optional[str]:
        ...
        # Look for "2c. Content of topics P1 to P9" - find the ACTUAL content, not TOC entry
        pattern = r'2c\.\s+Content\s+of\s+topics\s+P1\s+to\s+P[6789]'
        matches = list(re.finditer(pattern, self.pdf_text, re.IGNORECASE))
        
        content_start = None
        for match in matches:
            pos = match.start()
            # Check if this is actual content (has "Topic P1:" nearby) vs TOC (has page number)
            ...
            # Actual content has "Topic P1:" or "P1.1" nearby
            if re.search(r'Topic\s+P1:', after_text, re.IGNORECASE) or re.search(r'P1\.1\s+', after_text):
                content_start = pos
        ...
        if not content_start:
            # Fallback: look for first "Topic P1:" occurrence that's NOT in TOC
            match = re.search(r'Topic\s+P1:\s+Matter', self.pdf_text, re.IGNORECASE)
            if match:
                content_start = match.start()
                print(f"[DEBUG] Found content section using fallback: Topic P1: Matter")
            else:
        ...
        snippet_file = self.debug_dir / f"J249-content-section.txt"
    ...
    def _extract_topic(self, topic_code: str, topic_name: str, parent_code: str, topic_num: int) -> List[Dict]:
        """Extract a topic (P1-P7) with all its content."""
        topics = []
        
        # Level 1: Topic (e.g., "Topic P1: Matter")
        ...
        for pattern in topic_patterns:
            ...
            if match:
                # Verify this is actual content (has sub-topic like "P1.1" nearby), not TOC
                after_text = self.content_section[match.start():match.start() + 2000]
                if re.search(rf'{re.escape(topic_code)}\.\d+', after_text):  # Has sub-topic like "P1.1"
        ...
        next_topic_pattern = rf"Topic\s+P{topic_num + 1}:" if topic_num < 7 else None
        if next_topic_pattern:
            ...
            if next_match:
                ...
                if re.search(rf'P{topic_num + 1}\.\d+', next_after):
        ...
        snippet_file = self.debug_dir / f"J249-{safe_code}-section.txt"
    ...
    def _extract_table_content(self, section_text: str, topic_code: str, topic_name: str) -> Optional[str]:
        ...
        prompt = f"""You are extracting educational content from a Physics GCSE specification document.
...
- Learning outcomes (with codes like P1.1a, P1.1b)
- To include (lists of items separated by commas)

Extract the content using this structure:
- Level 2: Sub-topic headings (e.g., "P1.1 The particle model")
- Level 3: Learning outcomes (e.g., "P1.1a describe the main features of the particle model...")
...
1.1 P1.1 The particle model
1.1.1 P1.1a describe the main features of the particle model
1.1.1.1 particles are very small
1.1.1.2 particles are in constant motion
1.1.2 P1.1b explain the differences between solids, liquids and gases
...
Begin extraction:"""
        ...
        if result:
            safe_code = re.sub(r'[^\w\-]', '_', topic_code)[:50]
            ai_file = self.debug_dir / f"J249-{safe_code}-ai-output.txt"
    ...
    def _parse_table_content(self, text: str, parent_code: str, topic_code: str) -> List[Dict]:
        ...
        for line in text.split('\n'):
            ...
            if 'overview' in title_lower and not re.search(r'P\d+\.\d+[a-z]', title):
