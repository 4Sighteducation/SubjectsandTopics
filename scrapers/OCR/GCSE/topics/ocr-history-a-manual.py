"""
OCR GCSE History A (J410) Manual Scraper
=========================================

Extracts topics from OCR GCSE History A J410 specification.

Structure:
- Level 0: Component Groups (Component Group 1, Component Group 2, Component Group 3)
- Level 1: Main studies (Period study, ALL depth studies, ALL thematic studies, ALL British depth studies)
- Level 2+: Detailed content from each study

CRITICAL: 
- DO NOT create "History A (Explaining the Modern World) (J410)" as Level 0 - start with Component Groups
- Extract ALL options, not just one:
  * Component Group 1: Period study (International Relations) + ALL 5 non-British depth studies
  ...
  * Component Group 2: ALL 3 thematic studies
  ...
  * Component Group 3: ALL 3 British depth studies + historic environments
...
Usage:
    python ocr-history-a-manual.py
"""
...
PDF_URL = 'https://www.ocr.org.uk/Images/207163-specification-accredited-gcse-history-a-first-teaching-2019-with-first-assessment-2021-j410.pdf'
SUBJECT_CODE = 'J410'
SUBJECT_NAME = 'History A (Explaining the Modern World)'

COMPONENT_GROUPS = [
    {
        'code': '1',
        'name': 'Component Group 1 (01–07): Period study and non-British depth study',
        'section_pattern': r'Content\s+of\s+the\s+period\s+study\s+\(J410/01-07\)',
        'studies': [
            {
                'type': 'period',
                'name': 'International Relations: the changing international order 1918–1975',
                'pattern': r'International\s+Relations:\s+the\s+changing\s+international\s+order\s+1918[–-]1975',
            },
            {
                'type': 'depth',
                'name': 'China 1950–1981: The People and the State',
                'pattern': r'China\s+1950[–-]1981:\s+The\s+People\s+and\s+the\s+State',
            },
            {
                'type': 'depth',
                'name': 'Germany 1925–1955: The People and the State',
                'pattern': r'Germany\s+1925[–-]1955:\s+The\s+People\s+and\s+the\s+State',
            },
            {
                'type': 'depth',
                'name': 'South Africa 1960–1994: The People and the State',
                'pattern': r'South\s+Africa\s+1960[–-]1994:\s+The\s+People\s+and\s+the\s+State',
            },
            {
                'type': 'depth',
                'name': 'The USA 1919–1948: The People and the State',
                'pattern': r'The\s+USA\s+1919[–-]1948:\s+The\s+People\s+and\s+the\s+State',
            },
            {
                'type': 'depth',
                'name': 'The USA 1945–1974: The People and the State',
                'pattern': r'The\s+USA\s+1945[–-]1974:\s+The\s+People\s+and\s+the\s+State',
            },
        ]
    },
    {
        'code': '2',
        'name': 'Component Group 2 (08–10): British Thematic study',
        'section_pattern': r'Content\s+of\s+the\s+British\s+thematic\s+studies\s+\(08-10\)',
        'studies': [
            {
                'type': 'thematic',
                'name': 'Migration to Britain c.1000 to c.2010',
                'pattern': r'Migration\s+to\s+Britain\s+c\.1000\s+to\s+c\.2010',
            },
            {
                'type': 'thematic',
                'name': 'Power: Monarchy and Democracy in Britain c.1000 to 2014',
                'pattern': r'Power:\s+Monarchy\s+and\s+Democracy\s+in\s+Britain\s+c\.1000\s+to\s+2014',
            },
            {
                'type': 'thematic',
                'name': 'War and British Society c.790 to c.2010',
                'pattern': r'War\s+and\s+British\s+Society\s+c\.790\s+to\s+c\.2010',
            },
        ]
    },
    {
        'code': '3',
        'name': 'Component Group 3 (11–13): British depth study and a study of the historic environment',
        'section_pattern': r'Content\s+of\s+the\s+British\s+depth\s+studies\s+\(11-13\)',
        'studies': [
            {
                'type': 'depth',
                'name': 'The Impact of Empire on Britain 1688–c.1730',
                'pattern': r'The\s+Impact\s+of\s+Empire\s+on\s+Britain\s+1688[–-]c\.1730',
            },
            {
                'type': 'historic',
                'name': 'Urban Environments: Patterns of Migration',
                'pattern': r'Urban\s+Environments:\s+Patterns\s+of\s+Migration',
            },
            {
                'type': 'depth',
                'name': 'The English Reformation c.1520–c.1550',
                'pattern': r'The\s+English\s+Reformation\s+c\.1520[–-]c\.1550',
            },
            {
                'type': 'historic',
                'name': 'Castles: Form and Function c.1000–1750',
                'pattern': r'Castles:\s+Form\s+and\s+Function\s+c\.1000[–-]1750',
            },
            {
                'type': 'depth',
                'name': 'Personal Rule to Restoration 1629–1660',
                'pattern': r'Personal\s+Rule\s+to\s+Restoration\s+1629[–-]1660',
            },
        ]
    },
]
...
class HistoryAScraper:
    """Scraper for OCR GCSE History A."""
    ...
    def scrape_all(self):
        """Scrape all topics."""
        print("\n" + "📜 "*40)
        print("OCR GCSE HISTORY A SCRAPER")
        print("📜 "*40)
    ...
    def _find_group_section(self, group: Dict) -> Optional[str]:
        ...
        for match in matches:
            ...
            if re.search(r'Learners|Content|Study|Period|Depth|Thematic', after_text, re.IGNORECASE):
                # Extract large section (500k chars or until next major section)
                start_pos = pos
                end_patterns = [
                    r'\n\s*Content\s+of\s+the\s+British\s+thematic',
                    r'\n\s*Content\s+of\s+the\s+British\s+depth',
                    r'\n\s*Content\s+of\s+the\s+study\s+of\s+the\s+historic',
    ...
    def _find_study_section(self, study: Dict, group_section: Optional[str]) -> Optional[str]:
        ...
        for match in matches:
            ...
            # Check if actual content
            if re.search(r'Learners|Content|Study|will|should|understand|explore', after_text, re.IGNORECASE):
                # Extract section (200k chars or until next study)
                start_pos = pos
                end_patterns = [
                    r'\n\s*China\s+1950',
                    r'\n\s*Germany\s+1925',
                    r'\n\s*South\s+Africa\s+1960',
                    r'\n\s*The\s+USA\s+1919',
                    r'\n\s*The\s+USA\s+1945',
                    r'\n\s*Migration\s+to\s+Britain',
                    r'\n\s*Power:\s+Monarchy',
                    r'\n\s*War\s+and\s+British\s+Society',
                    r'\n\s*The\s+Impact\s+of\s+Empire',
                    r'\n\s*The\s+English\s+Reformation',
                    r'\n\s*Personal\s+Rule\s+to\s+Restoration',
                    r'\n\s*Urban\s+Environments',
                    r'\n\s*Castles:\s+Form',
                    r'\n\s*Content\s+of\s+the',
                    r'\n\s*2d\.\s+Prior',
                    r'\n\s*3\.\s+Assessment',
                ]
    ...
    def _extract_with_ai(self, text: str, parent_code: str, study_name: str, study_type: str) -> Optional[str]:
        ...
        prompt = f"""Extract the complete topic hierarchy from this OCR GCSE History A specification section.
...
1. DO NOT create "History A (Explaining the Modern World) (J410)" or subject name as Level 0 - component groups are already Level 0, studies are Level 1
2. Extract ALL content with FULL depth - don't skip any topics, sub-topics, or learning objectives

3. STRUCTURE REQUIREMENTS:
   Level 2: Main topics/sections (e.g., time periods, themes, major events)
   Level 3: Sub-topics (e.g., specific events, policies, developments)
...
4. OUTPUT FORMAT: Numbered hierarchy (NOT markdown)
   ...
   Format:
   1. Level 2 Topic (e.g., "Conflict and co-operation 1918–1939" or "c.1000–1500")
   1.1. Level 3 Sub-topic (e.g., "The Versailles Peace Settlement" or "Population diversity in England before 1066")
   ...
   - Extract ALL sub-topics and specific examples
...
Extract the complete hierarchy now:"""
    ...
    def _parse_hierarchy(self, text: str, base_code: str) -> List[Dict]:
        ...
        excluded_patterns = [
            r'^history\s+a\s+\(explaining\s+the\s+modern\s+world\)\s+\(j410\)$',
            r'^history\s+a$',
            r'^subject:',
        ]
