...
REPORTS_DIR = Path(__file__).parent / "reports" / "alevel"
REPORTS_DIR.mkdir(parents=True, exist_ok=True)


class EduqasALevelUniversalScraper:
    """Universal scraper for all Eduqas A-Level courses."""
...
- A-Level specifications typically have MORE depth than GCSE - ensure you extract ALL available detail
...
- **Look for:**
  ...
  - Research studies, theories, methodologies → Level 4+
  - Evaluation points and critical analysis → Level 4+
...
- A-Level content is typically MORE detailed than GCSE - ensure you capture this depth
...
{content_section[:150000]}"""
        ...
        if AI_PROVIDER == "openai":
            try:
                response = openai_client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": "You are an expert at extracting curriculum content from specification documents. Extract the complete hierarchical structure with all details. A-Level specifications typically have more depth than GCSE - ensure you extract ALL available detail."},
    ...
    def _self_assess(self, subject: Dict, analysis: Dict, topics: List[Dict], level_counts: Dict) -> Dict:
        ...
        # Check topic count (A-Level typically has more topics than GCSE)
        if len(topics) < 20:
            issues.append("Very few topics extracted")
            grade -= 30
        elif len(topics) < 100:
            warnings.append("Relatively few topics extracted - A-Level typically has 100-500+ topics")
            grade -= 10
    ...
    def _upload_topics(self, subject: Dict, topics: List[Dict]) -> bool:
        ...
        try:
            ...
            supabase.table('staging_aqa_topics').delete().eq('subject_id', subject_id).eq('exam_board', 'WJEC').execute()
