"""
OCR GCSE Biology A (Gateway Science) Manual Scraper
====================================================

Extracts topics from OCR GCSE Biology A (Gateway Science) J247 specification.

Structure (as explained on page 12):
- Level 0: Subject
- Level 1: Topics (B1-B6, B7 for practical skills)
  - Format: "Topic B1: Cell level systems"
- Level 2: Sub-topics
  - Format: "B1.1 Cell structures"
- Level 3: Learning outcomes (with reference codes)
  - Format: "B1.1a", "B1.1b", etc.
...
Extract only from "2c. Content of topics B1 to B6" section.

Requirements:
    pip install requests pdfplumber openai anthropic

Usage:
    python ocr-biology-a-manual.py
"""
...
PDF_URL = 'https://www.ocr.org.uk/Images/234594-specification-accredited-gcse-gateway-science-suite-biology-a-j247.pdf'
SUBJECT_CODE = 'J247'
SUBJECT_NAME = 'Biology A'

# Topics B1-B6 + B7 (practical skills)
TOPICS = [
    'B1: Cell level systems',
    'B2: Scaling up',
    'B3: Organism level systems',
...
class BiologyAScraper:
    """Scraper for OCR GCSE Biology A (Gateway Science)."""
    ...
    def scrape_all(self):
        """Scrape all topics."""
        print("\n" + "🧬 "*40)
        print("OCR GCSE BIOLOGY A (GATEWAY SCIENCE) SCRAPER")
        print("🧬 "*40)
        ...
        if not content_section:
            print("[ERROR] Could not find content section (2c. Content of topics B1 to B6)")
            return False
        ...
        # Extract each topic (B1-B7)
        for topic_idx, topic_name in enumerate(TOPICS):
            topic_code = topic_name.split(':')[0].strip()  # e.g., "B1"
            print(f"\n{'='*80}")
    ...
    def _find_content_section(self) -> Optional[str]:
        ...
        # Look for "2c. Content of topics B1 to B6" - find the ACTUAL content, not TOC entry
        # The TOC entry is just "2c. Content of topics B1 to B6 10" (with page number)
        # The actual content starts on page 10 with "2c. Content of topics B1 to B6" as a heading
        
        # First, find all occurrences of "2c. Content" to distinguish TOC from actual content
        pattern = r'2c\.\s+Content\s+of\s+topics\s+B1\s+to\s+B[67]'
        matches = list(re.finditer(pattern, self.pdf_text, re.IGNORECASE))
        
        content_start = None
        for match in matches:
            pos = match.start()
            # Check if this is actual content (has "Topic B1:" nearby) vs TOC (has page number)
            ...
            # Actual content has "Topic B1:" or "B1.1" nearby
            if re.search(r'Topic\s+B1:', after_text, re.IGNORECASE) or re.search(r'B1\.1\s+', after_text):
                content_start = pos
        ...
        if not content_start:
            # Fallback: look for first "Topic B1:" occurrence that's NOT in TOC
            match = re.search(r'Topic\s+B1:\s+Cell\s+level\s+systems', self.pdf_text, re.IGNORECASE)
            if match:
                content_start = match.start()
                print(f"[DEBUG] Found content section using fallback: Topic B1: Cell level systems")
            else:
        ...
        snippet_file = self.debug_dir / f"J247-content-section.txt"
    ...
    def _extract_topic(self, topic_code: str, topic_name: str, parent_code: str, topic_num: int) -> List[Dict]:
        """Extract a topic (B1-B7) with all its content."""
        topics = []
        
        # Level 1: Topic (e.g., "Topic B1: Cell level systems")
        ...
        for pattern in topic_patterns:
            ...
            if match:
                # Verify this is actual content (has sub-topic like "B1.1" nearby), not TOC
                after_text = self.content_section[match.start():match.start() + 2000]
                if re.search(rf'{re.escape(topic_code)}\.\d+', after_text):  # Has sub-topic like "B1.1"
        ...
        next_topic_pattern = rf"Topic\s+B{topic_num + 1}:" if topic_num < 7 else None
        if next_topic_pattern:
            ...
            if next_match:
                ...
                if re.search(rf'B{topic_num + 1}\.\d+', next_after):
        ...
        snippet_file = self.debug_dir / f"J247-{safe_code}-section.txt"
    ...
    def _extract_table_content(self, section_text: str, topic_code: str, topic_name: str) -> Optional[str]:
        ...
        prompt = f"""You are extracting educational content from a Biology GCSE specification document.
...
- Learning outcomes (with codes like B1.1a, B1.1b)
- To include (lists of items separated by commas)

Extract the content using this structure:
- Level 2: Sub-topic headings (e.g., "B1.1 Cell structures")
- Level 3: Learning outcomes (e.g., "B1.1a describe how light microscopes...")
...
1.1 B1.1 Cell structures
1.1.1 B1.1a describe how light microscopes and staining can be used to view cells
1.1.1.1 lenses
1.1.1.2 stage
1.1.1.3 lamp
1.1.1.4 use of slides and cover slips
1.1.2 B1.1b explain how sub-cellular structures relate to their functions
1.1.2.1 nucleus: contains genetic material
1.1.2.2 mitochondria: site of aerobic respiration
...
Begin extraction:"""
        ...
        if result:
            safe_code = re.sub(r'[^\w\-]', '_', topic_code)[:50]
            ai_file = self.debug_dir / f"J247-{safe_code}-ai-output.txt"
    ...
    def _parse_table_content(self, text: str, parent_code: str, topic_code: str) -> List[Dict]:
        ...
        for line in text.split('\n'):
            ...
            if 'overview' in title_lower and not re.search(r'B\d+\.\d+[a-z]', title):
