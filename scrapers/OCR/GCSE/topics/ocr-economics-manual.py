"""
OCR GCSE Economics (J205) Manual Scraper
========================================

Extracts topics from OCR GCSE Economics J205 specification.

Structure:
- Level 0: Components (Component 01, Component 02)
- Level 1: Main topics (e.g., "1. Introduction to Economics")
- Level 2: Sub-topics from "Topic" column (e.g., "1.1 Main economic groups...")
- Level 3: Items from "Learners should be able to" column

CRITICAL: 
- DO NOT create "Economics (J205)" as Level 0 - start with components
- Extract ALL items from "Learners should be able to" column as Level 3
- Components should be Level 0, not Level 1
...
PDF_URL = 'https://www.ocr.org.uk/Images/306377-specification-accredited-gcse-economics-j205.pdf'
SUBJECT_CODE = 'J205'
...
COMPONENTS = [
    {
        'code': '01',
        'name': 'Component 01: Introduction to Economics',
        'section_pattern': r'2c\.\s+Content\s+of\s+J205/01',
    },
    {
        'code': '02',
        'name': 'Component 02: National and International Economics',
        'section_pattern': r'2c\.\s+Content\s+of\s+J205/02',
    },
]


class EconomicsScraper:
    """Scraper for OCR GCSE Economics."""
    ...
    def scrape_all(self):
        """Scrape all topics."""
        print("\n" + "💰 "*40)
        print("OCR GCSE ECONOMICS SCRAPER")
        print("💰 "*40)
        ...
        if level2_topics:
            print(f"\n[DEBUG] Sample Level 2 topics ({len(level2_topics)} total):")
            for t in level2_topics[:5]:
                print(f"  - {t['code']}: {t['title'][:60]}")
        else:
            print("\n[WARN] No Level 2 topics found!")
    ...
    def _find_component_section(self, component: Dict) -> Optional[str]:
        ...
        for match in matches:
            ...
            # Check if actual content (has descriptive text) vs TOC (has page number)
            if re.search(r'Learners\s+should|Topic|introduces|develops', after_text, re.IGNORECASE):
                content_start = pos
                print(f"[DEBUG] Found actual content at position {pos}")
                break
            elif re.search(r'\d+\s*$', before_text[-50:]):  # Page number = TOC
        ...
        end_patterns = [
            r'\n\s*2c\.\s+Content\s+of\s+J205/',  # Next component
        ...
        print(f"[DEBUG] Saved section snippet ({len(section)} chars)")
    ...
    def _extract_with_ai(self, text: str, parent_code: str, component_name: str) -> Optional[str]:
        ...
        prompt = f"""Extract the complete topic hierarchy from this OCR GCSE Economics specification section.

COMPONENT: {component_name}
PARENT CODE: {parent_code}

CRITICAL REQUIREMENTS:
1. DO NOT create "Economics (J205)" or subject name as Level 0 - components are already Level 0
2. Extract from the table structure with columns:
   - Column 1: "Topic" (e.g., "1.1 Main economic groups...")
   - Column 2: "Learners should be able to" (bullet points)

3. STRUCTURE REQUIREMENTS:
   Level 1: Main topics (e.g., "1. Introduction to Economics" or "3. Economic objectives and the role of government")
   Level 2: Sub-topics from "Topic" column (e.g., "1.1 Main economic groups..." or "3.1 Economic growth")
   Level 3: ALL items from "Learners should be able to" column
     * Extract EVERY bullet point from this column
     * Each bullet point becomes a Level 3 item
     * Match each Level 3 item to its corresponding Level 2 topic

4. OUTPUT FORMAT: Numbered hierarchy (NOT markdown)
   CRITICAL: Use numbered format ONLY, NOT markdown headers (### or ####)
   CRITICAL: You MUST include Level 1 AND Level 2 topics - don't skip them!
   
   Format:
   1. Level 1 Topic (main section, e.g., "3. Economic objectives and the role of government")
   1.1. Level 2 Topic (from "Topic" column, e.g., "3.1 Economic growth")
   1.1.1. Level 3 Item (from "Learners should be able to" column - first bullet)
   1.1.2. Level 3 Item (from "Learners should be able to" column - second bullet)
   1.1.3. Level 3 Item (from "Learners should be able to" column - third bullet)
   1.2. Level 2 Topic (next topic from "Topic" column, e.g., "3.2 Low unemployment")
   1.2.1. Level 3 Item (first bullet for this topic)
   1.2.2. Level 3 Item (second bullet for this topic)
   (continue for ALL topics and ALL bullets)
   
   IMPORTANT: 
   - ALWAYS include Level 1 topics (main sections)
   - ALWAYS include Level 2 topics (sub-topics from Topic column)
   - THEN add Level 3 items (from Learners should be able to column)
   ...
   USE ONLY:
   - Numbered format: "1.", "1.1.", "1.1.1.", etc.

5. DEPTH REQUIREMENTS:
   - Extract ALL "Learners should be able to" items - don't skip any
   - Create Level 3 items for every bullet point
    ...
    def _parse_hierarchy(self, text: str, base_code: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            if markdown_match:
                ...
                # If Level 2 and no parent found, find or create Level 1 parent
                elif level == 2 and parent_code is None:
                    # Extract Level 1 number (e.g., from "3.1" get "3")
                    level1_number = parts[0]
                    # Check if Level 1 exists in parent_stack
                    if 1 in parent_stack:
                        parent_code = parent_stack[1]
                    else:
                        # Create Level 1 topic implicitly
                        level1_code = f"{base_code}_{level1_number}"
                        # Try to find Level 1 title from previous lines or use generic
                        level1_title = f"{level1_number}. Topic"  # Will be updated if we find it
                        level1_topic = {
                            'code': level1_code,
                            'title': level1_title,
                            'level': 1,
                        ...
                        all_topics.append(level1_topic)
                        parent_stack[1] = level1_code
