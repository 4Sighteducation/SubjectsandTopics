"""
OCR GCSE Geography A (J383) Manual Scraper
===========================================

Extracts topics from OCR GCSE Geography A J383 specification.

Structure:
- Level 0: Components (Component 01, Component 02, Component 03)
- Level 1: Sections (e.g., "Landscapes of the UK", "People of the UK")
- Level 2: Topics/Key Ideas (from "Key Ideas" column in tables)
- Level 3+: Content items (from "Content" column in tables - detailed learning objectives)

CRITICAL: 
- DO NOT create "Geography A (Geographical Themes) (J383)" as Level 0 - start with Components
- Extract ALL content from the "Content" column in tables as Level 3+
- Tables have columns: "Section", "Key Ideas", "Content", "Scale"
- Extract every item from the "Content" column - these are detailed learning objectives

Requirements:
    pip install requests pdfplumber openai anthropic

Usage:
    python ocr-geography-a-manual.py
"""
...
PDF_URL = 'https://www.ocr.org.uk/Images/207306-specification-accredited-gcse-geography-a-j383.pdf'
SUBJECT_CODE = 'J383'
SUBJECT_NAME = 'Geography A (Geographical Themes)'

COMPONENTS = [
    {
        'code': '01',
        'name': 'Component 01: Living in the UK Today',
        'section_pattern': r'Content\s+of\s+Living\s+in\s+the\s+UK\s+Today\s+\(J383/01\)',
    },
    {
        'code': '02',
        'name': 'Component 02: The World Around Us',
        'section_pattern': r'Content\s+of\s+The\s+World\s+Around\s+Us\s+\(J383/02\)',
    },
    {
        'code': '03',
        'name': 'Component 03: Geographical Skills',
        'section_pattern': r'Content\s+of\s+Geographical\s+Skills\s+\(J383/03\)',
    },
]


class GeographyAScraper:
    """Scraper for OCR GCSE Geography A."""
    ...
    def scrape_all(self):
        """Scrape all topics."""
        print("\n" + "🗺️ "*40)
        print("OCR GCSE GEOGRAPHY A SCRAPER")
        print("🗺️ "*40)
    ...
    def _find_component_section(self, component: Dict) -> Optional[str]:
        ...
        for match in matches:
            ...
            if re.search(r'Section|Key\s+Ideas|Content|table|This\s+component', after_text, re.IGNORECASE):
                content_start = pos
        ...
        end_patterns = [
            r'\n\s*Content\s+of\s+The\s+World\s+Around\s+Us',  # Next component
            r'\n\s*Content\s+of\s+Geographical\s+Skills',  # Next component
            r'\n\s*Content\s+of\s+J383/',  # Next component (alternative format)
    ...
    def _extract_with_ai(self, text: str, parent_code: str, component_name: str) -> Optional[str]:
        ...
        prompt = f"""Extract the complete topic hierarchy from this OCR GCSE Geography A specification section.

COMPONENT: {component_name}
PARENT CODE: {parent_code}

CRITICAL REQUIREMENTS:
1. DO NOT create "Geography A (Geographical Themes) (J383)" or subject name as Level 0 - components are already Level 0
2. Extract from the table structure with columns: "Section", "Key Ideas", "Content", "Scale"

3. STRUCTURE REQUIREMENTS:
   Level 1: Sections (e.g., "Landscapes of the UK", "People of the UK", "UK Environmental Challenges")
   Level 2: Topics/Key Ideas from the "Key Ideas" column (e.g., "The physical landscapes of the UK have distinctive characteristics.")
   Level 3: Content items from the "Content" column - extract EVERY item from this column
     * The "Content" column contains detailed learning objectives
     * Extract ALL bullet points, sub-items, and detailed points from the Content column
     * Each distinct learning objective or point becomes a Level 3 item
     * If Content has multiple sentences or bullet points, split them into separate Level 3 items
   Level 4: Further sub-items if Content column has nested structure
...
4. OUTPUT FORMAT: Numbered hierarchy (NOT markdown)
   ...
   Format:
   1. Level 1 Section (e.g., "Landscapes of the UK")
   1.1. Level 2 Topic/Key Idea (from "Key Ideas" column, e.g., "The physical landscapes of the UK have distinctive characteristics.")
   1.1.1. Level 3 Content Item (from "Content" column - first learning objective)
   1.1.2. Level 3 Content Item (from "Content" column - second learning objective)
   1.1.3. Level 3 Content Item (from "Content" column - third learning objective)
   1.2. Level 2 Topic/Key Idea (next Key Idea from table)
   1.2.1. Level 3 Content Item (first Content item for this Key Idea)
   1.2.2. Level 3 Content Item (second Content item for this Key Idea)
   (continue for ALL sections, ALL Key Ideas, and ALL Content items)
   
   IMPORTANT: 
   - ALWAYS include Level 1 sections
   - ALWAYS include Level 2 Topics/Key Ideas
   - THEN extract ALL Level 3 items from the Content column
...
5. CONTENT COLUMN EXTRACTION:
   ...
   - If Content has bullet points or lists, extract each as separate Level 3 items
...
Extract the complete hierarchy now:"""
    ...
    def _parse_hierarchy(self, text: str, base_code: str) -> List[Dict]:
        ...
        excluded_patterns = [
            r'^geography\s+a\s+\(geographical\s+themes\)\s+\(j383\)$',
            r'^geography\s+a$',
        ]
