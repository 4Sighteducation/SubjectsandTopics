"""
OCR Vocational (Cambridge Technicals / Cambridge Nationals) - Common Helpers
===========================================================================
Shared parsing + AI extraction + Supabase staging upload helpers.

We upload into the existing staging tables used across the repo:
- staging_aqa_subjects
- staging_aqa_topics

Those table names are historical; they contain multi-board data.
"""
...
def call_ai(provider: str, client, *, prompt: str, max_tokens: int = 16000) -> str:
    # Simple retry loop for transient provider errors (rate limits, overloads).
...
def extract_pdf_text(pdf_content: bytes) -> str:
    ...
    # Silence noisy pdfminer warnings (e.g. "CropBox missing from /Page, defaulting to MediaBox")
...
def replace_subject_topics(
    ...
    *,
    ...
    # Clear old topics for this subject+board
    sb.table("staging_aqa_topics").delete().eq("subject_id", subject_id).eq("exam_board", exam_board).execute()
    ...
    for i in range(0, len(payload), batch_size):
        ...
        res = sb.table("staging_aqa_topics").insert(batch).execute()
        inserted_all.extend(res.data or [])
