"""
BATCH INTERNATIONAL GCSE PAPER SCRAPER
=======================================

Runs the universal scraper for all International GCSE subjects with progress tracking.

Usage:
    python batch-igcse-papers.py
    python batch-igcse-papers.py --resume  # Resume from checkpoint
"""
...
# All International GCSE subjects to scrape
IGCSE_SUBJECTS = [
    'IG-Accounting',
    'IG-Arabic',
    'IG-Biology',
...
class BatchPaperScraper:
    ...
    def __init__(self, resume=False):
        ...
        self.log_file = self.output_dir / f"batch-papers-{self.timestamp}.log"
        self.checkpoint_file = self.output_dir / "checkpoint-papers.json"
        self.summary_file = self.output_dir / f"summary-papers-{self.timestamp}.json"
    ...
    def run_batch(self):
        """Run all subjects."""
        self._log("="*80)
        self._log("BATCH INTERNATIONAL GCSE EXAM PAPER SCRAPER")
        self._log("="*80)
        self._log(f"Total subjects: {len(IGCSE_SUBJECTS)}")
        self._log(f"Already completed: {len(self.checkpoint['completed'])}")
        self._log(f"To process: {len([s for s in IGCSE_SUBJECTS if s not in self.checkpoint['completed']])}")
        self._log("="*80)
        
        scraper_script = self.script_dir / "universal-igcse-paper-scraper.py"
        
        for idx, subject_code in enumerate(IGCSE_SUBJECTS, 1):
            # Skip if already done
            if subject_code in self.checkpoint['completed']:
                self._log(f"[{idx}/{len(IGCSE_SUBJECTS)}] ✓ SKIP: {subject_code} - already completed")
                continue
            
            self._log(f"\n[{idx}/{len(IGCSE_SUBJECTS)}] PROCESSING: {subject_code}")
            self._log("-"*80)
    ...
    def _save_summary(self):
        ...
        summary = {
            'batch_name': 'International GCSE Exam Papers Batch',
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Batch International GCSE paper scraper')
    parser.add_argument('--resume', action='store_true', help='Resume from checkpoint')
    args = parser.parse_args()
    
    print("\n" + "📄 "*20)
    print("BATCH INTERNATIONAL GCSE EXAM PAPER SCRAPER")
    print("📄 "*20)
    print(f"\nThis will scrape exam papers for all {len(IGCSE_SUBJECTS)} International GCSE subjects")
