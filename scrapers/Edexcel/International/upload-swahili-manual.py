"""
Upload Manual Swahili Hierarchy
================================

Uploads the manually created Swahili hierarchy with proper encoding.

Usage:
    python upload-swahili-manual.py
"""
...
SUBJECT_INFO = {
    ...
    'exam_board': 'Edexcel',
    'pdf_url': 'https://qualifications.pearson.com/content/dam/pdf/International%20GCSE/Swahili/2017/specification-and-sample-assessments/international-gcse-swahili-specification.pdf'
}
...
def upload_to_supabase(subject_info: Dict, topics: List[Dict]) -> bool:
    ...
    try:
        ...
        # Verify
        print(f"\n[INFO] Verifying Swahili text in database...")
        check = supabase.table('staging_aqa_topics').select('topic_name').eq(
            'subject_id', subject_id
        ).ilike('topic_name', '%Swahili%').limit(1).execute()
        
        if check.data:
            print(f"✅ VERIFIED: Swahili text found in database!")
...
def main():
    print("="*60)
    print("UPLOAD MANUAL SWAHILI HIERARCHY")
    print("="*60)
    
    # Parse hierarchy file
    hierarchy_file = Path(__file__).parent / "adobe-ai-output" / "swahili-manual-hierarchy.txt"
    ...
    if success:
        ...
        print("✅ SUCCESS!")
        print("="*60)
        print(f"Uploaded {len(topics)} Swahili topics")
