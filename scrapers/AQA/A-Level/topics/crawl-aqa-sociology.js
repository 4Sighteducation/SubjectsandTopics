/**
 * AQA SOCIOLOGY A-LEVEL Scraper
 * Code: 7192
 * Standard numbered hierarchy (3.1 → 3.1.1 → bullets)
 * 3 sections: Education, Topics in Sociology, Crime and Deviance
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/sociology/a-level/sociology-7192/specification/subject-content'
};

async function crawlSociology() {
  console.log('🔍 Crawling Sociology...');
  try {
...
function parseSociologyTopics(pages) {
  console.log('\n📋 Parsing Sociology topics...');
  const allTopics = [];
...
async function main() {
  console.log('🚀 SOCIOLOGY');
  console.log('='.repeat(60));
  try {
    const pages = await crawlSociology();
    const topics = parseSociologyTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ SOCIOLOGY COMPLETE! Topics:', topics.length);
  } catch (error) {
