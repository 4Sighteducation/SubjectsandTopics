/**
 * AQA SPANISH A-LEVEL Scraper
 * Code: 7692
 * Standard numbered hierarchy (3.1 → 3.1.1 → bullets)
 * Exclude: 3.5 Individual research project
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/spanish/a-level/spanish-7692/specification/subject-content'
};

async function crawlSpanish() {
  console.log('🔍 Crawling Spanish...');
  try {
    ...
    for (let i = 0; i < contentUrls.length; i++) {
      ...
      // Skip 3.5 Individual research project
      if (slug.includes('individual-research') || slug.includes('research-project')) {
...
function parseSpanishTopics(pages) {
  console.log('\n📋 Parsing Spanish topics...');
  const allTopics = [];
...
async function main() {
  ...
  try {
    const pages = await crawlSpanish();
    const topics = parseSpanishTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ SPANISH COMPLETE! Topics:', topics.length);
  } catch (error) {
