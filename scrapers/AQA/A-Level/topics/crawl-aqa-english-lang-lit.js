/**
 * AQA ENGLISH LANGUAGE AND LITERATURE A-LEVEL Scraper
 * Code: 7707
 * 3 sections: Telling stories, Exploring conflict, Making connections
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/english/a-level/english-7707/specification/subject-content'
};

// Standard crawl
async function crawlEnglishLangLit() {
  console.log('🔍 Crawling English Language and Literature...');
  try {
...
function parseEnglishLangLitTopics(pages) {
  console.log('\n📋 Parsing topics...');
  const allTopics = [];
...
async function main() {
  console.log('🚀 ENGLISH LANGUAGE AND LITERATURE');
  console.log('='.repeat(60));
  try {
    const pages = await crawlEnglishLangLit();
    const topics = parseEnglishLangLitTopics(pages);
