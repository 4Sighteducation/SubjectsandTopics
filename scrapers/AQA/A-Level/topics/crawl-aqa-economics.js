/**
 * AQA ECONOMICS A-LEVEL Scraper
 * Code: 7136
 * Table format (like Accounting/Business)
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/economics/a-level/economics-7136/specification/subject-content'
};

// Copy Accounting crawl logic
async function crawlEconomics() {
  console.log('🔍 Crawling Economics...');
  try {
...
function parseEconomicsTopics(pages) {
  console.log('\n📋 Parsing Economics topics...');
  const allTopics = [];
...
function parseTablePage(markdown, mainCode) {
  ...
  for (const line of lines) {
    if (line.includes('Content') && line.includes('Additional')) inTable = true;
...
async function main() {
  ...
  try {
    const pages = await crawlEconomics();
    const topics = parseEconomicsTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ ECONOMICS COMPLETE! Topics:', topics.length);
  } catch (error) {
