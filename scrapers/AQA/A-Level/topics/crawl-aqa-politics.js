/**
 * AQA POLITICS A-LEVEL Scraper
 * Code: 7152
 * Standard numbered hierarchy (3.1 → 3.1.1 → bullets)
 * 3 sections: UK Politics, USA Politics, Political Ideas
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/politics/a-level/politics-7152/specification/subject-content'
};

async function crawlPolitics() {
  console.log('🔍 Crawling Politics...');
  try {
...
function parsePoliticsTopics(pages) {
  console.log('\n📋 Parsing Politics topics...');
  const allTopics = [];
...
async function main() {
  ...
  try {
    const pages = await crawlPolitics();
    const topics = parsePoliticsTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ POLITICS COMPLETE! Topics:', topics.length);
  } catch (error) {
