/**
 * AQA MEDIA STUDIES A-LEVEL Scraper
 * Code: 7572
 * Standard numbered hierarchy (3.2 → 3.2.1 → bullets)
 * Exclude: 3.1 (Close study products) and 3.8 (Non-exam assessment)
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/media-studies/a-level/media-studies-7572/specification/subject-content'
};

async function crawlMediaStudies() {
  console.log('🔍 Crawling Media Studies...');
  try {
    ...
    for (let i = 0; i < contentUrls.length; i++) {
      ...
      // Skip 3.1 and 3.8
      if (slug.includes('close-study') || slug.includes('non-exam')) {
...
function parseMediaTopics(pages) {
  console.log('\n📋 Parsing Media Studies topics...');
  const allTopics = [];
...
async function main() {
  console.log('🚀 MEDIA STUDIES');
  console.log('='.repeat(60));
  try {
    const pages = await crawlMediaStudies();
    const topics = parseMediaTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ MEDIA STUDIES COMPLETE! Topics:', topics.length);
  } catch (error) {
