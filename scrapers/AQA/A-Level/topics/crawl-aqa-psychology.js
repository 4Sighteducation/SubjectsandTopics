/**
 * AQA PSYCHOLOGY A-LEVEL Scraper
 * Code: 7182
 * Standard numbered hierarchy (3.1 → 3.1.1 → bullets)
 * 3 sections: Introductory topics, Psychology in context, Issues and options
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/psychology/a-level/psychology-7182/specification/subject-content'
};

async function crawlPsychology() {
  console.log('🔍 Crawling Psychology...');
  try {
...
function parsePsychologyTopics(pages) {
  console.log('\n📋 Parsing Psychology topics...');
  const allTopics = [];
...
async function main() {
  ...
  try {
    const pages = await crawlPsychology();
    const topics = parsePsychologyTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ PSYCHOLOGY COMPLETE! Topics:', topics.length);
  } catch (error) {
