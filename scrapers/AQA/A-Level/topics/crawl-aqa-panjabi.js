/**
 * AQA PANJABI A-LEVEL Scraper
 * Code: 7682
 * Standard numbered hierarchy (3.1 → 3.1.1 → bullets)
 * Exclude: 3.5 Research topic
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/panjabi/a-level/panjabi-7682/specification/subject-content'
};

async function crawlPanjabi() {
  console.log('🔍 Crawling Panjabi...');
  try {
    ...
    for (let i = 0; i < contentUrls.length; i++) {
      ...
      // Skip 3.5 Research topic
      if (slug.includes('research-topic')) {
...
function parsePanjabiTopics(pages) {
  console.log('\n📋 Parsing Panjabi topics...');
  const allTopics = [];
...
async function main() {
  ...
  try {
    const pages = await crawlPanjabi();
    const topics = parsePanjabiTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ PANJABI COMPLETE! Topics:', topics.length);
  } catch (error) {
