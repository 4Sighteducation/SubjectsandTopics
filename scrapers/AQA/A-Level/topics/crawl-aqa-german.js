/**
 * AQA GERMAN A-LEVEL Scraper
 * Code: 7662
 * Simple numbered hierarchy (3.1 → 3.1.1 → bullets)
 * 5 main sections: Social issues, Political/artistic culture, Grammar, Works, Research project
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/german/a-level/german-7662/specification/subject-content'
};

// Standard crawl
async function crawlGerman() {
  console.log('🔍 Crawling German...');
  try {
...
function parseGermanTopics(pages) {
  console.log('\n📋 Parsing German topics...');
  const allTopics = [];
...
async function main() {
  ...
  try {
    const pages = await crawlGerman();
    const topics = parseGermanTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ GERMAN COMPLETE! Topics:', topics.length);
  } catch (error) {
