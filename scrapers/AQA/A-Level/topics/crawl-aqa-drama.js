/**
 * AQA DRAMA A-LEVEL Scraper
 * Code: 7262
 * 
 * 5 main sections: 3.1-3.5 with content
 */
...
const SUBJECT = {
  ...
  baseUrl: 'https://www.aqa.org.uk/subjects/drama/a-level/drama-7262/specification/subject-content'
};

// Standard crawl - map and scrape sub-pages
async function crawlDrama() {
  console.log('🔍 Crawling Drama...');
  try {
...
// Parse numbered hierarchy + bullets
function parseDramaTopics(pages) {
  console.log('\n📋 Parsing Drama topics...');
  
  const allTopics = [];
  for (const page of pages) {
    const markdown = page.markdown || '';
    const pageTopics = parseNumberedPage(markdown);
...
function parseNumberedPage(markdown) {
  const topics = [];
  const lines = markdown.split('\n');
  let currentCode = null;
  
  for (const line of lines) {
    // Numbered headings (3.1, 3.1.1, etc.)
    ...
    if (bulletMatch && currentCode) {
      const bulletCode = `${currentCode}.${topics.filter(t => t.parentCode === currentCode).length + 1}`;
      topics.push({ code: bulletCode, title: bulletMatch[1].trim(), level: currentCode.split('.').length - 1, parentCode: currentCode });
    }
  }
...
async function main() {
  ...
  try {
    const pages = await crawlDrama();
    const topics = parseDramaTopics(pages);
    await uploadToStaging(topics);
    console.log('\n✅ DRAMA COMPLETE! Topics:', topics.length);
  } catch (error) {
