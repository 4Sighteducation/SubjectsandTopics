"""
WJEC AS/A Level - Universal Past Papers Scraper (Past Papers / Mark Schemes tab)

WJEC qualification pages render an accordion of documents under:
  https://www.wjec.co.uk/en/qualifications/<slug>-asa-level/#tab_pastpapers

Those accordions link to PDFs hosted on:
  https://pastpapers.download.wjec.co.uk/...

We parse:
  - year + series from either /S24/ folder OR filename prefix (s24-/w19-)
  - component_code from filename (uppercased), stripping mark-scheme/report suffixes and date stamps
  - doc type from link text (Question / Mark Scheme / Examiner Report)

We group into paper sets:
  key = (year, exam_series, component_code)
"""
...
def _classify_doc_type(title: str) -> str | None:
    ...
    if "resource" in t:
        # treat resources as part of the question paper set (no dedicated column)
        return "question_paper"
    return None
...
def _parse_series_from_url(url: str) -> tuple[int | None, str | None]:
    ...
    series_map = {"S": "Summer", "W": "Winter", "N": "November", "J": "January"}

    m = re.search(r"/([A-Za-z])(\d{2})/", clean)
    if m:
        code = m.group(1).upper()
        yy = int(m.group(2))
        return 2000 + yy, series_map.get(code, code)
...
def _parse_component_from_url(url: str) -> tuple[int | None, str | None]:
    ...
    # Infer a "paper number" from U1/U2/U3... or P1/P2.. if present, otherwise default 1.
    pn = None
    um = re.search(r"\bU(\d)\b", doc)
    if um:
        pn = int(um.group(1))
    pm = re.search(r"\bP(\d)\b", doc)
    if pm:
        pn = int(pm.group(1))
    return pn, doc
...
def scrape_wjec_alevel_papers(subject_code: str, subject_name: str, pastpapers_url: str, headless: bool = True) -> list[dict]:
    print("=" * 80)
    print("WJEC A-LEVEL - PAPERS SCRAPER")
    print("=" * 80)
