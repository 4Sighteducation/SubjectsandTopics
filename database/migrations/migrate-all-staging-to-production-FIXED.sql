-- MIGRATE ALL STAGING DATA TO PRODUCTION (SCHEMA-CORRECT VERSION)
...
-- ========================================
-- STEP 3: Mark old subjects as archived
-- ========================================
DO $$
DECLARE
...
-- ========================================
-- STEP 5: Create mapping table for ALL boards
-- ========================================
...
-- Handle duplicate topic names by appending topic_code when needed
WITH duplicate_check AS (
  ...
  GROUP BY sim.production_id, st.topic_name, st.topic_level
),
topics_with_unique_names AS (
  SELECT 
    ...
    st.topic_code,
    CASE 
      WHEN dc.dup_count > 1 THEN st.topic_name || ' [' || st.topic_code || ']'
      ELSE st.topic_name
    END as topic_name,
    st.topic_level,
    ROW_NUMBER() OVER (PARTITION BY st.subject_id ORDER BY st.topic_level, st.topic_code, st.id) as sort_order
  FROM staging_aqa_topics st
  JOIN temp_subject_id_mapping sim ON st.subject_id = sim.staging_id
  LEFT JOIN duplicate_check dc ON 
    dc.production_id = sim.production_id 
    AND dc.topic_name = st.topic_name 
    AND dc.topic_level = st.topic_level
)
...
ON CONFLICT (exam_board_subject_id, topic_name, topic_level) DO UPDATE SET
...
-- ========================================
DROP TABLE temp_subject_id_mapping;
